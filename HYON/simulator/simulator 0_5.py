...
def run():
    try:
        ...
        wind_Vx = -wind_V * cos(wind_rad_yz) #進行方向がプラス (入力のとき)
        ...
        msgbox.showerror(title="Error!",message="Where is the savepath?")
...
def show_graph():
    ...
    for i in range (1000):
            try:
                x_val.append(data['x [m]'][i])
                y_val.append(data['y [m]'][i])

                plt.xlim(0,data['x [m]'].max()*1.5)
                plt.ylim(0,data['y [m]'].max()*1.1)
                plt.scatter(x_val,y_val,color='black',marker='.')
                plt.pause(0.0001)
            except Exception:
                break
    msgbox.showinfo(title="infomation",message="やった")
    plt.show()
...
def show_instructions():
    ...
    instrction=open("シミュレーター説明書.txt","r",encoding='utf-8')
...
simul=Tk()
simul.title("Rocket Simulator 0.5")
...
T_total_text=Label(input_text_frame,text="Total Thrust [N*s]")
...
a=Button(savepath_frame,text='graph',command=show_graph)
a.pack()
