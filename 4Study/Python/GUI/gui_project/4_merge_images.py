...
def add_file():
    files=filedialog.askopenfilenames(title="이미지 파일을 선택하세요",\
        ...
        initialdir=r"C:\Users\h\OneDrive\바탕 화면\PythonWorkspace\gui_basic")  #최초에 경로  r로 탈출문자 사용
...
def browse_dest_path():   #폴더
    folder_selected=filedialog.askdirectory()
    if folder_selected=='': #사용자가 취소
        return
    txt_dest_path.delete(0,END)  #if text -> "1.0",END
...
txt_dest_path.pack(side="left",fill="x",padx=5,pady=5,expand=True,ipady=4) #높이변경
...
def merge_image():
    ...
    dest_path=path.join(txt_dest_path.get(),"combined.jpg")
    result_img.save(dest_path)
    msgbox.showinfo(title="알림",message="작업이 완료되었습니다")
...
def start():
    ...
    # 파일 목록확인
    if list_file.size()==0:
        msgbox.showwarning(title="경고",message="이미지 파일을 추가하세요")
        return
    # 저장 경로 확인
    if len(txt_dest_path.get())==0:
        msgbox.showwarning(title="경고",message="저장 경로를 선택하세요")
        return
    merge_image()
